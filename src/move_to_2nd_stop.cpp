...
int main(int argc, char** argv) {
	ros::init(argc, argv, "pipette_tool_action_client_move_to_2nd_stop");
	...
	// Move to 2nd Stop
	ROS_INFO_STREAM("Sending Goal: Move to 2nd Stop @ " << speed << " m/s");
	goal.command.action = goal.command.ACTION_MOVE_TO_2ND_STOP;
	goal.command.volume = 0.0;
	...
	ROS_INFO("Successfull Move to 2nd Stop");
	return 0;
}
