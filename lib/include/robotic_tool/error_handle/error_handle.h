...
#include <robotic_tool/error_handle/error_code.h>
...
public:
	...
	const char* getCodeString(uint32_t offset_err) {
		...
		else if (offset_err >= OFFSET_PWM_PFC_ERROR_CODE && offset_err < OFFSET_PWM_PFC_ERROR_CODE + (uint32_t)PwmPfcErrCode::ALWAYS_AT_END) {
			err = offset_err - OFFSET_PWM_PFC_ERROR_CODE;
			return getErrCodeString(PwmPfcErrCode(err));
		}
